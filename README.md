...
- **Go to Definition**: Jump to the definition of variables, functions, classes, and imports defined in `<script>` blocks — works from usages within `<script>` and from `{{ interpolations }}` in `<template>`
